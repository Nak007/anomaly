...
def optimize_condition(X, y, n_max=10):
    ...
    meaning that once `variable` is determined, `X` will be
    reduced to only `non-target` sample according to previous 
    `cutoff`. Then, the process continues until `f1_score` 
    stops improving.
    ...
    \t `variable`, any value that is more than or equal to 
    \t 99 is defined as `anomaly`.
